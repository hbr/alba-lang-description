...
In order to define an inductive type we need a class name, optional formal
...
Every inductive type must have at least one basic constructor. All base
...
the concepts apply to all other inductive types.
...
The compiler generates for each inductive type an equality function. E.g. for
...
I.e. objects of an inductive type can only be equal if they have been
...
This mathematical fact is call _injectivity_ of constructors: Two objects of
an inductive type can only be equal if they have been constructed by the same
constructors with the same arguments.

**Inversion**:
On the other hand if two objects of an inductive type have been constructed
...
equality of two objects of an inductive type. However the above definition of
...
generates for any inductive type an inheritance relation so that the type
inherits `ANY`.

A module which defines inductive types must use the module `any` either
...
The compiler generates for each inductive type an induction law. E.g. for
...
The induction principle for any inductive type can be generated mechanically
by the following pattern.

    all(p: {T})           -- 'T' is the inductive type, possibly generic
        require
...
Suppose that `X` is an inductive type and we want to prove the assertion
...
Since `X` is an inductive type the compiler could extract the goal predicate
...
principle of the corresponding inductive type.
